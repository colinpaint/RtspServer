...
void* xop::Alloc (uint32_t size) { return MemoryManager::Instance().Alloc (size); }
...
void MemoryPool::Init (uint32_t size, uint32_t n) {
  ...
  for (uint32_t num = 1; num < num_blocks_; n++) {
    MemoryBlock* next = (MemoryBlock*)(memory_ + (num * (block_size_ + sizeof(MemoryBlock))));
    next->block_id = num + 1;
    next->pool = this;
    next->next = nullptr;
