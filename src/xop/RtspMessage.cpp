...
//}}}
std::string RtspRequest::GetAuthResponse() const { return auth_response_; }
//{{{
...
int RtspResponse::BuildSetupTcpReq (const char* buf, int buf_size, int trackId) {
  ...
  snprintf ((char*)buf, buf_size,
      ...
      "Session: %s\r\n"
      "\r\n",
      rtsp_url_.c_str(), trackId, interleaved[0], interleaved[1], this->GetCSeq() + 1,
      user_agent_.c_str(), this->GetSession().c_str());
