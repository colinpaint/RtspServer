...
//}}}
//{{{
bool RtspRequest::ParseAccept (string& message) {

  if ((message.rfind ("Accept") == string::npos)
      || (message.rfind ("sdp") == string::npos)) {
    return false;
    }
...
//}}}
//{{{
bool RtspRequest::ParseTransport (string& message) {

  size_t pos = message.find ("Transport");
  if (pos != string::npos) {
    if ((pos=message.find ("RTP/AVP/TCP")) != string::npos) {
    ...
    else if  ((pos = message.find ("RTP/AVP")) != string::npos) {
      uint16_t rtp_port = 0, rtcpPort = 0;
      if (((message.find ("unicast", pos)) != string::npos)) {
        transport_ = RTP_OVER_UDP;
        if(sscanf(message.c_str()+pos, "%*[^;];%*[^;];%*[^=]=%hu-%hu", &rtp_port, &rtcpPort) != 2) {
          return false;
          }
        }
      else if((message.find ("multicast", pos)) != string::npos) {
...
//}}}
//{{{
bool RtspRequest::ParseSessionId (string& message) {
...
//}}}
//{{{
bool RtspRequest::ParseAuthorization (string& message) {

  size_t pos = message.find ("Authorization");
  if (pos != string::npos) {
    if ((pos = message.find ("response=")) != string::npos) {
...
//}}}
//{{{
string RtspRequest::GetIp() const {
...
//}}}
//{{{
string RtspRequest::GetRtspUrl() const {
...
//}}}
//{{{
string RtspRequest::GetRtspUrlSuffix() const {
...
//}}}
string RtspRequest::GetAuthResponse() const { return auth_response_; }
//{{{
uint8_t RtspRequest::GetRtpChannel() const {

  auto iter = header_line_param_.find ("rtp_channel");
  if (iter != header_line_param_.end()) {
    return uint8_t(iter->second.second);
    }
...
uint16_t RtspRequest::GetRtpPort() const {
  ...
  if (iter != header_line_param_.end()) {
    return uint16_t(iter->second.second);
    }
...
int RtspRequest::BuildDescribeRes (const char* buf, int buf_size, const char* sdp) {
  ...
  snprintf ((char*)buf, buf_size,
    ...
    "\r\n"
    "%s",
    this->GetCSeq(), (int)strlen(sdp), sdp);

  return (int)strlen(buf);
  }
//}}}
//{{{
int RtspRequest::BuildSetupMulticastRes (const char* buf, int buf_size, const char* multicast_ip,
  ...
  snprintf ((char*)buf, buf_size,
    ...
    "Session: %u\r\n"
    "\r\n",
    this->GetCSeq(), multicast_ip, this->GetIp().c_str(), port, session_id);
...
int RtspResponse::BuildAnnounceReq (const char* buf, int buf_size, const char *sdp) {
  ...
  snprintf ((char*)buf, buf_size,
    ...
    "\r\n"
    "%s",
    rtsp_url_.c_str(), this->GetCSeq() + 1, user_agent_.c_str(), this->GetSession().c_str(), (int)strlen(sdp), sdp);
...
int RtspResponse::BuildSetupTcpReq (const char* buf, int buf_size, int trackId) {
  ...
  snprintf ((char*)buf, buf_size,
    ...
    this->GetCSeq() + 1, user_agent_.c_str(), this->GetSession().c_str());
